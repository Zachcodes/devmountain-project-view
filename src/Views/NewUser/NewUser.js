...
export default class NewUser extends Component {
    ...
    componentDidMount() {
        axios.get('/api/students/info').then( res => {
            let {student, cohorts, cohortTypes} = res.data
            let o = this.setSelectedCohortAndType(student.cohort, cohorts)
            let {selectedType, selectedCohortId} = o
            this.setState({student, cohorts, cohortTypes, selectedType, selectedCohortId})
        })
    }
    ...
    render() {
        let {about, email, first, github, image, last, linkedin, portfolio} = this.state.student
        let {cohortTypes, selectedType, selectedCohortId, cohorts} = this.state
        ...
        return (
            <div>
                <form onSubmit={this.handleSubmit}>
                    ...
                    <div>Email: <input value={email} onChange={(e) => this.handleStudentChange(e.target.value, 'email')}/></div>
